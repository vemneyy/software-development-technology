    ...
    while (true) {
        try {
            ...
            if (abs(a - b) < c + d && abs(a - c) < b + d && abs(a - d) < b + c &&
                ...
                cout << "Данного четырёхугольника с заданными сторонами не существует" << std::endl;
                break;
            }

            cout << "Введённая фигура - " << nameFigure << std::endl;
        }

        catch (...) {
            cout << "Введена буква. Ожидалось число" << std::endl;
        }
    }
