...
int main() {
        ...
        while (!validInput) {
            ...
            if (isValid) {
                try {
                    ...
                    if (i == 1 && vars[1] == 0) {
                        std::cout << "Ошибка: деление на ноль \nПожалуйста, введите ненулевое значение для y!" << std::endl;
                        validInput = false; // устанавливаем флаг в false, чтобы пройти цикл ввода для y еще раз
                    }

                    else if (i == 0 && vars[0] < 0) {
                        std::cout << "Ошибка: отрицательное значение под квадратным корнем! \nПожалуйста, введите неотрицательное значение для x!" << std::endl;
                        validInput = false; // устанавливаем флаг в false, чтобы пройти цикл ввода для y еще раз
                    }

                    else if (vars[0] > std::numeric_limits<double>::max()) {
                        std::cerr << "Ошибка: переполнение переменной. \nПопробуйте ввести меньшее значение!" << std::endl;
                        validInput = false;
                    }
                    ...
                    std::cout << "Ошибка: некорректный ввод. \nВводите только числа с разделителем в виде запятой!" << std::endl;
                }
            }
            else {
                std::cout << "Некорректный ввод. \nВведите число с разделителем в виде запятой!" << std::endl;
            }
        }
    ...
    double temp_final = sqrt(vars[0]) * (1 + temp);

    // Проверка на бесконечность и NaN
    if (!std::isfinite(temp_final)) {
        if (std::isnan(temp_final)) {
    ...
    double t = sqrt(vars[0]) * ( 1 + ( pow(vars[2], 2) / (3 * vars[2] - (vars[0] * vars[1] / 5) ) ) );
